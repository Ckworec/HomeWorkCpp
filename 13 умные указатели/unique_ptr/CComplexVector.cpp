...
{
    len = num;
    m_Real = std::make_unique<double []>(len);
    m_Imaginary = std::make_unique<double []>(len);
...
CComplexVector :: CComplexVector(const CComplexVector &vect) : CComplexVector(vect.len)
{
        for (int k = 0; k < len; ++k)
        {
            m_Real[k] = vect.m_Real[k];
